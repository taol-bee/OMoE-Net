...
def save_image_tensor(image_tensor, output_path="output/"):
    # print("input image_tensor shape:", image_tensor.shape)
    if image_tensor.dim() == 4 and image_tensor.size(0) == 1:
        image_tensor = image_tensor[0]  # 去除 batch 维

    image_np = torch_to_np(image_tensor)
    # print("image_np shape:", image_np.shape)
    image_np = np.clip(image_np, 0, 1)
    # print("image_np min/max:", image_np.min(), image_np.max())
...
def torch_to_np(img_var):
    ...
    """
    np_img = img_var.detach().cpu().numpy()
    if np_img.ndim == 4:
        np_img = np_img[0]
    return np_img
