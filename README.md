...
> **Abstract:** *Unified image restoration aims to recover high-quality images from diverse degradations within a shared model. However, existing methods often suffer from unstable gains or even performance drop as the diversity of training degradations increases, due to optimization conflicts and representation interference in shared parameters. To address this issue, we propose OMoE-Net, a unified restoration framework based on orthogonally disentangled mixture-of-experts. OMoE-Net combines a shared expert branch for modeling degradation-invariant restoration priors, an orthogonally disentangled MoE module for promoting parameter-level expert specialization, and a degradation-aware path controller for dynamically selecting relevant experts while reducing feature redundancy among routed representations. In this way, the proposed framework jointly enhances common-prior modeling and degradation-specific restoration within a unified encoder-decoder architecture. Extensive experiments on multiple benchmark datasets and degradation settings show that OMoE-Net achieves consistently superior restoration performance and maintains stronger stability as degradation diversity increases.* 
<hr />
